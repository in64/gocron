#!/usr/bin/env python3
"""枚举 gocron Linux 双架构发布实际使用的 tracked 源文件。"""
...
GENERATED_FILES = {
    "web/gocronx-admin/.auto-import.json",
    "web/gocronx-admin/src/types/import/auto-imports.d.ts",
    "web/gocronx-admin/src/types/import/components.d.ts",
}
...
def selected_go_files(root: pathlib.Path, goarch: str) -> tuple[set[str], set[str]]:
    generated = root / "web/gocronx-admin/dist"
...
def is_generated(relative: str) -> bool:
    return relative in GENERATED_FILES or relative.startswith(GENERATED_PREFIXES)


def is_untracked_build_input(relative: str, package_directories: set[str]) -> bool:
    if is_generated(relative):
        return False
    if relative in EXPLICIT_FILES or is_frontend_input(relative):
        return True
    ...
    if path.suffix in GO_SOURCE_SUFFIXES and parent in package_directories:
        return True
    return (
        relative.startswith(("docs/zh/guide/", "docs/en/guide/"))
        and path.suffix == ".md"
    ) or relative in {"docs/zh/index.md", "docs/en/index.md"}
